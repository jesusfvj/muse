...
} from "../../Utils/uploadSongsFunctions";
import { ProfileLoader } from "../Pages/Profile/ProfileLoader";
import { useUI } from "../../Context/UI/UIContext";

export const FormUploadedSongs = ({
  selectedFiles,
  setSelectedFiles,
  setShowUploadSongsModal,
}) => {
    ...
    if (selectedFiles) {
      ...
      Object.values(selectedFiles).map((selectedFile, index) => {
        ...
        copyRegisterData = {
          ...copyRegisterData,
          [`genre${index + 1}`]: "",
          [`songTitle${index + 1}`]: selectedFile.name.substring(0, selectedFile.name.length - 4),
        };
      });
    ...
    <div className="w-[95%] h-full flex flex-col">
      <div className="m-2 flex flex-col sm:flex-row justify-center items-center gap-4">
        <label className="flex gap-4">
          ...
          />
          <Typography text="Is it an album?" type="p1" color="white" />
        </label>
        {isAlbumChecked && (
          <InputWithLabel
            name="albumName"
